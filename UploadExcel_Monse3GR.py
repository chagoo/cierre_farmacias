...
def aplicacion():
   ...
   ### se comentariza 03042025 por qwue no cumple con el calculo de completado, esto debido que solo tomando un completado ya lo pone como estatus general
   ## cosa que no funciona asi, deben estar el mismo ceco pra casa departamento con estatus todos con fecha ini es decir firmados todos los departamentso
    ...
    query = text("SELECT Ceco AS orden_ceco, MAX(FechaIni) AS FechaIni, CASE WHEN EXISTS (SELECT 1 FROM CierreSucursales4 CS WHERE CS.Ceco = CierreSucursales4.Ceco AND CS.Tipo_General = CierreSucursales4.Tipo_General AND (CS.FechaFin IS NULL OR CS.FechaFin = '') AND Departamento <> 'BAJA DIRECTA') THEN '-' ELSE CONVERT(VARCHAR, MAX(FechaFin), 23) END AS FechaFin, CASE WHEN EXISTS (SELECT 1 FROM CierreSucursales4 CS WHERE CS.Ceco = CierreSucursales4.Ceco AND CS.Tipo_General = CierreSucursales4.Tipo_General AND (CS.FechaFin IS NULL OR CS.FechaFin = '') AND Departamento <> 'BAJA DIRECTA') THEN 'Iniciado' ELSE 'Completado' END AS Estatus_General, Tipo_General FROM CierreSucursales4 GROUP BY Ceco, Tipo_General")
...
if __name__ == '__main__':
           app.run(debug=True, port=5020, host='10.30.45.88')
